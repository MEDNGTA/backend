...
def json_reg():
 ...
 if request.method == 'POST':
    ...
    user=Users(username=username, password=password, email=email, firstname=firstname, lastname=lastname, phonenumber=phonenumber, token=token)
    db.session.add(user)
    db.session.commit()        
...
def json_login():
 if request.method == 'POST':
    ...
    if username or email:
       ...
       log_obj_un= Users.query.filter_by(username=username).first() #verify if the username exist on database
       log_obj_em= Users.query.filter_by(email=email).first() 
...
def update_reg():
 if request.method == 'POST':
    ...
    email = req_data['email'] 
    user= Users.query.filter_by(email=email,token=token).first() #select user
    if user :
